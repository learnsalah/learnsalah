<script>
    ...
    import {fajr, dhuhr, asr, maghrib, isha} from "../../../data/prayers/prayers"
    ...
    let selectedPrayer = 'fajr';  // Default selected prayer
    let prayer_type = $page.url.searchParams.get('type');
    ...
    const startAutoPlay = () => {
        ...
        if (isAutoPlaying) {
            autoPlayIntervalId = setTimeout(() => {
                ...
                if (currentStageIndex < prayers[selectedPrayer].length - 1) {
                    currentStageIndex++;
                } else {
            ...
            }, prayers[selectedPrayer][currentStageIndex].duration);
        }
    };
    ...
    beforeUpdate(() => {
        // return if prayer invalid
        if (!Object.keys(prayers).includes(prayer_type)) {
            goto('/');
        }
    });
...
<div>
    ...
    <div class="container-fluid">
        <PrayerElementCounter prayer={prayers[selectedPrayer]} {currentStageIndex} />
        
        <div style="display:flex; justify-content:space-between; align-items:center;margin-bottom:15px; background-color:red;">
            <span style="text-transform:capitalize;">
                {prayer_type} Prayer - 
                {prayers[selectedPrayer][currentStageIndex].name}
            </span>
...
<div class="containerx">
    <div class="imageContainer">
        <img src="{prayers[selectedPrayer][currentStageIndex].img}" class="responsiveImage">
    </div>

    <div class="prayerArabicText">
        {prayers[selectedPrayer][currentStageIndex].text_arabic}
    </div>

    <div class="prayerTransliterationText">
        {#if prayers[selectedPrayer][currentStageIndex].count > 1}
        <!-- <code style="font-weight:900;font-size:medium;height:30px;margin-right:15px;">
            {prayers[selectedPrayer][currentStageIndex].count}&times;
        </code> -->
        {/if}
        {prayers[selectedPrayer][currentStageIndex].text_transliteration}
    </div>
    
    <div class="prayerEnglishText">
        {prayers[selectedPrayer][currentStageIndex].text_eng}
    </div>
</div>
